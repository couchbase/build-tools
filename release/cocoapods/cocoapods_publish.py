...
def update_podspec_file(args):
    ...
    package_dict = {'CouchbaseLite-Enterprise.podspec':
                    ...
                    'CouchbaseLiteVectorSearch.podspec':
                    'couchbase-lite-vector-search_xcframework'
                    }
    s3_url = 'https://packages.couchbase.com/releases'
    if args.file in package_dict:
        package_name = package_dict[args.file]
        pkg_url = '{}/{}/{}/{}_{}.zip'.format(
            s3_url, args.product, args.version, package_name, args.version)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Provision cocoapod podspec File",
        allow_abbrev=True)
    ...
    parser.add_argument('--file', help='Cocoapod podspec file', required=True)
