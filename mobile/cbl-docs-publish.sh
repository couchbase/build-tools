...
if [[ ${PRODUCT} == 'couchbase-lite-ios' ]]; then
    ...
    PACKAGE_NAME_ARRAY=("CouchbaseLite-${VERSION}-javadoc.jar")
fi
...
for PKG in ${PACKAGE_NAME_ARRAY[@]}; do
    ...
    if [[ ${PRODUCT} == 'couchbase-lite-android-ee' ]]; then
        ...
        unzip CouchbaseLite-${VERSION}-javadoc.zip -d ${PKG_NAME} || exit 1
        #Android:
        ...
        S3_URL_couchbase_lite_objc="s3://docs.couchbase.com/mobile/${VERSION}/couchbase-lite-swift"
        curl -LO ${S3_URL}/${PKG} || exit 1
        unzip ${PKG} -d ${PKG_NAME} || exit 1
        #Swift:
        s3cmd -c $S3CONFIG --acl-public -r put couchbase-lite-swift-documentation_enterprise_${VERSION}/ ${S3_URL_couchbase_lite_objc}/  || exit 1
        s3cmd -c $S3CONFIG ls ${S3_URL_couchbase_lite_objc}/
        echo "Re-uploading css file ..."
