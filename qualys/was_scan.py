...
def update_scan_url(qgc, current_time, args):
    ...
    '''
    web_name_tag = '<name>' + args.web_name + ' - ' + args.scan_type_name + \
        ' WAS Scan Launch From API - ' + current_time + '</name>' + '\n'
    web_url_tag = '<url>' + args.web_url + '</url>'
    parameters = request_xml_header + web_name_tag + \
        '\n' + web_url_tag + request_xml_footer
    ...
    if root.responseCode != 'SUCCESS':
        ...
        logger.info(
            'Successfully updated webapp: %s',
            root.data.WebApp.id.text)
...
def scan_report(qgc, current_time, args, scan_id):
    ...
    '''
    web_scan_name = '<name>' + args.web_name + ' - ' + args.scan_type_name + \
        ' WAS Scan Launch From API - ' + current_time + '</name>' + '\n'
    ...
    parameters = request_xml_header + web_scan_name + scan_type + \
        scan_id_content + content + profile_id + '\n' + request_xml_footer
    logger.debug('Scan parameters: %s', parameters)

    xml_output = qgc.request(call, parameters)
    root = objectify.fromstring(xml_output.encode('utf-8'))
    logger.debug(
        'Xml output from launching scan: %s',
        xml_output.encode('utf-8'))

    # Need to check update results
    if root.responseCode != 'SUCCESS':
        logger.error(
            'Error found when launching scan: %s',
            root.responseErrorDetails.errorMessage.text)
        sys.exit(1)
    else:
    ...
    while True:
        ...
        if scan_root.responseCode != 'SUCCESS':
            # Unable to obtain scan status if scan_id is invalid or scan is
            # deleted.
            logger.error(
                'Error found when getting scan result: %s',
                scan_root.responseErrorDetails.errorMessage.text)
            logger.error(
                'Scan result response code: %s',
                scan_root.responseCode)
            sys.exit(1)
        ...
        if count <= 310:
            if scan_root.data.WasScan.status != 'FINISHED':
                ...
                logger.info(
                    'Wait for scan to finish.  Current scan status: %s',
                    scan_root.data.WasScan.status)
                logger.info('Sleep for 60 seconds... %s', sleep_time)
            else:
                break
        else:
            logger.error(
                'Scan did not finish in expected time frame. aborting...')
            logger.error('Scan result: %s', xml_output.encode('utf-8'))
            sys.exit(1)

    try:
        logger.info(
            'Scan finished successfully! Scan id: %s',
            scan_root.data.WasScan.id.text)
        return scan_root.data.WasScan.id.text
    except AttributeError as error:
        logger.error(
            'Error found when looking up scan_root.data.WasScan.id.text: %s',
            error)
        sys.exit(1)
...
def get_report_status(qgc, report_id):
    ...
    try:
        ...
        logger.error(
            'Error found when returning root.data.Report.status: %s',
            error)
        sys.exit(1)
...
def generate_report(qgc, args, was_scan_id):
    ...
    '''
    scan_id_content = '<scans><WasScan><id>' + \
        was_scan_id + '</id></WasScan></scans>'
    parameters = request_xml_header + \
        scan_id_content + request_xml_footer

    xml_output = qgc.request(call, parameters)
    root = objectify.fromstring(xml_output.encode('utf-8'))
    logger.debug(
        'Output from generating report: %s',
        xml_output.encode('utf-8'))

    if root.responseCode != 'SUCCESS':
        logger.error('Error found when generating report: %s',
                     root.responseErrorDetails.errorMessage.text)
        sys.exit(1)
    else:
    ...
    if report_id:
        ...
        pdf_report_name = "Scan_Report_" + args.web_name + '_' + \
            args.bld_num + '_' + args.scan_type_name + '_' + report_id + ".pdf"
        with open(pdf_report_name, "wb") as report:
            report.write(output)
        logger.info(
            'Report has been downloaded successfully: %s',
            pdf_report_name)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Couchbase WAS Scan Automation\n\n",
        formatter_class=RawTextHelpFormatter)
    parser.add_argument(
        '--web-url',
        help="Host IP:port or Web URL:port.  e.g. http://172.23.104.255:8091\n",
        required=True)
    parser.add_argument(
        '--webapp-id',
        help="WebApp ID. e.g. CBServer5.0: 2695834, SGW1.5.0:3084099 \n",
        required=True)
    parser.add_argument(
        '--web-name',
        help="WebApp Name. e.g. SGW-1.5.0, Couchbase Server 5.0.0\n",
        required=True)
    parser.add_argument(
        '--profile-id',
        help="Profile id to scan\n",
        required=True)
    parser.add_argument(
        '--scan-type-name',
        help="VULNERABILITY or DISCOVERY\n",
        required=True)
    parser.add_argument(
        '--bld-num',
