...
class Blackduck:
    def __init__ (self):
        ...
        self.hub = HubInstance(bd_creds['url'], bd_creds['username'], bd_creds['password'], insecure=True)
    ...
    def get_notifications (self, filter_url):
        ...
        if response.status_code == 200:
            ...
            response = self.hub.execute_get( f"{notifications_url}?limit={limit}")
            if response.status_code == 200:
    ...
    def get_vulnerability_notifications (self, project_name, version_name, newer_than, older_than):
        notifications=self.get_notifications("filter=notificationType%3Avulnerability")

        #further reduce noises from project creations, unknown components, etc.
        vulnerability_notifications = [v for v in notifications if v['type'] == "VULNERABILITY"]
    ...
    def get_project_version_files(self, project_version_url):
        ...
        if response.status_code == 200:
            for item in response.json().get('items'):
                ...
                component_url=item['matches'][0]['component'].rsplit('/origins', 1)[0]
    ...
    # get vulnerabilities by project and version, filter out ignored, mitigated, patched, remediation complete, and duplicate
    def get_vulnerability_bom(self, project_version_url):
        url = project_version_url + '/vulnerability-bom?filter=remediationType%3Anew&filter=remediationType%3Aneeds_review&filter=remediationType%3Aremediation_required'
        response = self.hub.execute_get(url)
        if response.status_code == 200:
